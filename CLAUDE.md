...
The main codebase is in `sidd_ccc/` — always `cd sidd_ccc` before running commands.

```
cctV_cheating/
├── sidd_ccc/          # Main project
│   ├── core/          # 7-stage pipeline modules
│   ├── utils/         # Visualization and report generation
...
# Install dependencies (from sidd_ccc/)
cd sidd_ccc && pip install -r requirements.txt

# Run the pipeline (creates outputs/test1, test2, etc.)
cd sidd_ccc && python run_pipeline.py --video path/to/exam.mp4

# Run with custom config
cd sidd_ccc && python run_pipeline.py --video exam.mp4 --config config/settings.yaml --output results/
```

## Architecture

7-stage computer vision pipeline for detecting cheating in exam CCTV footage:
...
`sidd_ccc/config/settings.yaml` controls all parameters. Key tuning knobs for false positive reduction:
